...
def test_add_user_to_role(pg_client):
    # Test adding a user to an existing role
    pg_client.create_user('testuser', 'testpass')
    pg_client.create_role('testrole')
    pg_client.add_user_to_role('testrole', 'testuser')
    result = pg_client.execute_query("SELECT * FROM pg_auth_members WHERE roleid = (SELECT oid FROM pg_roles WHERE rolname = 'testrole') AND member = (SELECT oid FROM pg_roles WHERE rolname = 'testuser')")
    assert len(result) == 1

    # Test adding a user to a non-existing role
    with pytest.raises(Exception) as e:
        pg_client.add_user_to_role('nonexistingrole', 'testuser')
    assert 'role "nonexistingrole" does not exist' in str(e.value)

    # Test adding a non-existing user to an existing role
    with pytest.raises(Exception) as e:
        pg_client.add_user_to_role('testrole', 'nonexistinguser')
...
def test_add_users_to_role(pg_client):
    ...
    pg_client.create_user('testuser1', 'testpass')
    pg_client.create_user('testuser2', 'testpass')
    pg_client.create_role('testrole')
    pg_client.add_users_to_role('testrole', ['testuser1', 'testuser2'])
    result = pg_client.execute_query("SELECT * FROM pg_auth_members WHERE roleid = (SELECT oid FROM pg_roles WHERE rolname = 'testrole')")
    assert len(result) == 2

    # Test adding multiple users to a non-existing role
    with pytest.raises(Exception) as e:
        pg_client.add_users_to_role('nonexistingrole', ['testuser1', 'testuser2'])
    assert 'role "nonexistingrole" does not exist' in str(e.value)

    # Test adding a non-existing user to an existing role
    with pytest.raises(Exception) as e:
        pg_client.add_users_to_role('testrole', ['testuser1', 'nonexistinguser'])
    assert 'role "nonexistinguser" does not exist' in str(e.value)


def test_remove_user_from_role(pg_client):
    # Test removing a user from an existing role
    pg_client.create_user('testuser', 'testpass')
    pg_client.create_role('testrole')
    pg_client.add_user_to_role('testrole', 'testuser')
    pg_client.remove_user_from_role('testrole', 'testuser')
    result = pg_client.execute_query("SELECT * FROM pg_auth_members WHERE roleid = (SELECT oid FROM pg_roles WHERE rolname = 'testrole') AND member = (SELECT oid FROM pg_roles WHERE rolname = 'testuser')")
    assert len(result) == 0

    # Test removing a user from a non-existing role
    with pytest.raises(Exception) as e:
        pg_client.remove_user_from_role('nonexistingrole', 'testuser')
    assert 'role "nonexistingrole" does not exist' in str(e.value)

    # Test removing a non-existing user from an existing role
    with pytest.raises(Exception) as e:
        pg_client.remove_user_from_role('testrole', 'nonexistinguser')
    assert 'role "nonexistinguser" does not exist' in str(e.value)
...
def test_remove_users_from_role(pg_client):
    ...
    pg_client.create_role('testrole')
    pg_client.add_users_to_role('testrole', ['testuser1', 'testuser2'])
    pg_client.remove_users_from_role('testrole', ['testuser1', 'testuser2'])
    result = pg_client.execute_query("SELECT * FROM pg_auth_members WHERE roleid = (SELECT oid FROM pg_roles WHERE rolname = 'testrole')")
    assert len(result) == 0

    # Test removing multiple users from a non-existing role
    with pytest.raises(Exception) as e:
        pg_client.remove_users_from_role('nonexistingrole', ['testuser1', 'testuser2'])
    assert 'role "nonexistingrole" does not exist' in str(e.value)

    # Test removing a non-existing user from an existing role
    with pytest.raises(Exception) as e:
        pg_client.remove_users_from_role('testrole', ['testuser1', 'nonexistinguser'])
    assert 'role "nonexistinguser" does not exist' in str(e.value)
