...
def postgres_common_argument_spec():
    ...
    return dict(
        login_user=environ.get("PGUSER","postgres"),
        login_password=environ.get("PGPASSWORD",""),
        login_host=environ.get("PGHOST",""),
        login_unix_socket=environ.get("PGSOCKET",""),
        login_dbname=environ.get("PGDATABASE","postgres"),
        port=environ.get("PGPORT",""),
        ssl_mode=environ.get("PGSSLMODE","prefer"),
        ca_cert=environ.get("PGSSLROOTCERT",""),
        ssl_cert=environ.get("PGSSLCERT",""),
        ssl_key=environ.get("PGSSLKEY",""),
        connection_url=environ.get("PGURL",None),
    )
    ...
    try:
        ...
        if module.params.get('session_role'):
            ...
            try:
                ...
                logger.debug('Could not switch role: %s' % str(e))
            finally:
                cursor.close()

    except TypeError as e:
        if 'sslrootcert' in e.args[0]:
            logger.debug('Postgresql server must be at least '
...
def exec_sql(obj, query, query_params=None, return_bool=False, add_to_executed=True, dont_exec=False):
    ...
    try:
        ...
        logger.debug("Cannot execute SQL '%s': %s" % (query, str(e)))
    return False
...
def get_conn_params(module, params_dict, warn_db_default=True):
    ...
    if LooseVersion(psycopg2.__version__) >= LooseVersion('2.7.0'):
        if params_dict.get('db'):
            ...
            if warn_db_default:
                logger.debug('Database name has not been passed, '
    ...
    def __check_roles_exist(self):
            ...
            for group in self.groups:
                if group not in existent_groups:
                    if self.fail_on_role:
                        logger.debug("Role %s does not exist" % group)
                    else:
                        logger.debug("Role %s does not exist, pass" % group)
        ...
        for role in self.target_roles:
            if role not in existent_roles:
                if self.fail_on_role:
                    logger.debug("Role %s does not exist" % role)
                else:
                    logger.debug("Role %s does not exist, pass" % role)
                ...
                if role not in self.groups:
                    ...
                    if self.fail_on_role:
                        logger.debug("Role role '%s' is a member of role '%s'" % (role, role))
                    else:
                        logger.debug("Role role '%s' is a member of role '%s', pass" % (role, role))
